...
open class GameWordTypeMock: GameWordType, Mock {
    public init(sequencing sequencingPolicy: SequencingPolicy = .lastWrittenResolvedFirst, stubbing stubbingPolicy: StubbingPolicy = .wrap, file: StaticString = #file, line: UInt = #line) {
        SwiftyMockyTestObserver.setup()
        self.sequencingPolicy = sequencingPolicy
        self.stubbingPolicy = stubbingPolicy
    ...
    open func getLiveScoreObservable() -> Observable<LiveScore> {
        addInvocation(.m_getLiveScoreObservable)
		let perform = methodPerformValue(.m_getLiveScoreObservable) as? () -> Void
		perform?()
		var __value: Observable<LiveScore>
		do {
		    __value = try methodReturnValue(.m_getLiveScoreObservable).casted()
		} catch {
			onFatalFailure("Stub return value not specified for getLiveScoreObservable(). Use given")
			Failure("Stub return value not specified for getLiveScoreObservable(). Use given")
		}
		return __value
    }

    open func getQuestionObservablee() -> Observable<AttemptQuestion?> {
        addInvocation(.m_getQuestionObservablee)
		let perform = methodPerformValue(.m_getQuestionObservablee) as? () -> Void
		perform?()
		var __value: Observable<AttemptQuestion?>
		do {
		    __value = try methodReturnValue(.m_getQuestionObservablee).casted()
		} catch {
			onFatalFailure("Stub return value not specified for getQuestionObservablee(). Use given")
			Failure("Stub return value not specified for getQuestionObservablee(). Use given")
		}
		return __value
    }
    ...
    fileprivate enum MethodType {
        case m_startGame
        case m_getLiveScoreObservable
        case m_getQuestionObservablee

        static func compareParameters(lhs: MethodType, rhs: MethodType, matcher: Matcher) -> Matcher.ComparisonResult {
            switch (lhs, rhs) {
            case (.m_startGame, .m_startGame): return .match

            case (.m_getLiveScoreObservable, .m_getLiveScoreObservable): return .match

            case (.m_getQuestionObservablee, .m_getQuestionObservablee): return .match
            default: return .none
            }
        }
        ...
        func intValue() -> Int {
            ...
            case .m_getLiveScoreObservable: return 0
            case .m_getQuestionObservablee: return 0
            }
        }
        func assertionName() -> String {
            switch self {
            case .m_startGame: return ".startGame()"
            case .m_getLiveScoreObservable: return ".getLiveScoreObservable()"
            case .m_getQuestionObservablee: return ".getQuestionObservablee()"
            }
        }
    }
    ...
    open class Given: StubbedMethod {
        ...
        public static func getLiveScoreObservable(willReturn: Observable<LiveScore>...) -> MethodStub {
            return Given(method: .m_getLiveScoreObservable, products: willReturn.map({ StubProduct.return($0 as Any) }))
        }
        public static func getQuestionObservablee(willReturn: Observable<AttemptQuestion?>...) -> MethodStub {
            return Given(method: .m_getQuestionObservablee, products: willReturn.map({ StubProduct.return($0 as Any) }))
        }
        public static func getLiveScoreObservable(willProduce: (Stubber<Observable<LiveScore>>) -> Void) -> MethodStub {
            let willReturn: [Observable<LiveScore>] = []
			let given: Given = { return Given(method: .m_getLiveScoreObservable, products: willReturn.map({ StubProduct.return($0 as Any) })) }()
			let stubber = given.stub(for: (Observable<LiveScore>).self)
			willProduce(stubber)
			return given
        }
        public static func getQuestionObservablee(willProduce: (Stubber<Observable<AttemptQuestion?>>) -> Void) -> MethodStub {
            let willReturn: [Observable<AttemptQuestion?>] = []
			let given: Given = { return Given(method: .m_getQuestionObservablee, products: willReturn.map({ StubProduct.return($0 as Any) })) }()
			let stubber = given.stub(for: (Observable<AttemptQuestion?>).self)
			willProduce(stubber)
			return given
        }
    }
    ...
    public struct Verify {
        ...
        public static func getLiveScoreObservable() -> Verify { return Verify(method: .m_getLiveScoreObservable)}
        public static func getQuestionObservablee() -> Verify { return Verify(method: .m_getQuestionObservablee)}
    }
    ...
    public struct Perform {
        ...
        public static func getLiveScoreObservable(perform: @escaping () -> Void) -> Perform {
            return Perform(method: .m_getLiveScoreObservable, performs: perform)
        }
        public static func getQuestionObservablee(perform: @escaping () -> Void) -> Perform {
            return Perform(method: .m_getQuestionObservablee, performs: perform)
        }
    }
